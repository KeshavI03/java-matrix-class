		...
		try {
			for (int i = 0; i < addition.length(); i++) {
				for (int j = 0; j < addition.height(); j++) {
					returnArray[i][j] += addition.getElement(i, j);
				}
			}
			return new Matrix(returnArray);
		} catch (Exception e) {
		...
		try {
			for (int i = 0; i < subtraction.length(); i++) {
				for (int j = 0; j < subtraction.height(); j++) {
					returnArray[i][j] -= subtraction.getElement(i, j);
				}
			}
			return new Matrix(returnArray);
		} catch (Exception e) {
			...
			try {
				for (int i = 0; i < multiplication.length(); i++) {
					for (int j = 0; j < multiplication.height(); j++) {
						returnArray[i][j] *= multiplication.getElement(i, j);
					}
				}
				return new Matrix(returnArray);
			} catch (Exception e) {
		        ...
		        for(int i = 0; i < this.length(); i++) {
		            for (int j = 0; j < multiply.height(); j++) {
		                for (int k = 0; k < this.height(); k++) {
		                	returnArray[i][j] += array[i][k] * multiply.getElement(k,j);
		                }
		            }
		        }
		        return new Matrix(returnArray);
